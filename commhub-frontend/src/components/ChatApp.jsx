...
import { Hash, Lock, Search, Send, Smile, Paperclip, Plus, Phone, Video,
         ...
         CornerDownLeft, LogOut, Loader2, Info, Star, Copy, Eye,
         ExternalLink, Columns2, EyeOff, Bell, UserCircle, FileText } from 'lucide-react';
...
const fmtTime = t => { const n=parseFloat(t); return isNaN(n)?'':new Date(n).toLocaleTimeString([],{hour:'numeric',minute:'2-digit'}); };
const fmtDay  = t => {
...
function ContextMenu({ onClose, isDM, targetName }) {
  const items = [
    { icon: <Info size={16}/>,        label: 'Conversation details',  sub: true },
    { icon: <UserCircle size={16}/>,  label: isDM ? 'View full profile' : 'View members' },
    { icon: <Copy size={16}/>,        label: 'Copy link',             sub: true },
    { icon: <Star size={16}/>,        label: 'Star conversation' },
    { icon: <Bell size={16}/>,        label: 'Mute notifications' },
    null,
    { icon: <FileText size={16}/>,    label: 'Summarize conversation', sub: true },
    null,
    { icon: <Columns2 size={16}/>,    label: 'Open in split view' },
    { icon: <ExternalLink size={16}/>,label: 'Open in new window' },
    null,
    { icon: <EyeOff size={16}/>,      label: 'Hide conversation',     danger: true },
  ];
  return (
    <>
...
export default function ChatApp({ me, onLogout }) {
  ...
  /* ── Clear unread when channel becomes active ── */
  useEffect(() => {
    if(activeId) setUnread(p => { const n={...p}; delete n[activeId]; return n; });
  }, [activeId]);
    ...
    const onMsgNew = msg => {
      if(msg.channelId === activeId) {
        ...
        if('Notification' in window && Notification.permission==='granted' && document.hidden) {
          ...
          const n = new Notification(sender?.name || 'New message', {
  ...
  const sorted = useMemo(()=>[...messages].sort((a,b)=>parseFloat(a.t)-parseFloat(b.t)),[messages]);
  ...
  const sendMsg = (text, parentId=null) => {
    ...
    if(attachments.length && !parentId) {
      clean = clean ? clean + '\n' : '';
      clean += attachments.map(f=>`📎 ${f.name} (${(f.size/1024).toFixed(1)} KB)`).join('\n');
    }
  ...
  const onFileChange = (e) => {
    ...
    setAttachments(p=>[...p,...files.map(f=>({name:f.name,size:f.size,type:f.type}))]);
    e.target.value='';
  };
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden bg-white text-slate-800 antialiased"
      ...
      <nav className="flex w-16 shrink-0 flex-col items-center gap-3 bg-slate-900 py-4">
        ...
        <button className="grid h-10 w-10 place-items-center rounded-xl bg-slate-800 text-sm font-semibold text-slate-400 hover:bg-slate-700 hover:text-white"><Plus size={18}/></button>
      ...
      <aside className="flex w-64 shrink-0 flex-col bg-slate-800 text-slate-300">
        ...
        <div className="flex-1 overflow-y-auto px-2 py-3">
          ...
          {teammates.map(u => {
            ...
            const isActiveDM = activeDMUserId===u.id || (isDM && dmUserId===u.id);
            return (
              <button key={u.id} onClick={()=>openDM(u.id)}
                className={`flex w-full items-center gap-2.5 rounded-md px-2.5 py-1.5 text-sm transition ${isActiveDM?'bg-teal-500/15 text-white':cnt?'font-semibold text-white hover:bg-slate-700/60':'text-slate-300 hover:bg-slate-700/60'}`}>
      ...
      <main className="flex min-w-0 flex-1 flex-col bg-white">
        <header className="relative flex items-center justify-between border-b border-slate-200 px-5 py-3">
          <div className="flex min-w-0 items-center gap-2.5">
            {isDM
              ? <Avatar user={dmUser} size="h-7 w-7"/>
              : <span className="grid h-7 w-7 place-items-center rounded-md bg-slate-100 text-slate-600">{activeChannel?.type==='private'?<Lock size={15}/>:<Hash size={15}/>}</span>
            }
            <div className="min-w-0">
          ...
          <div className="flex items-center gap-1">
            ...
            <div className="relative ml-1">
              ...
              <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search messages"
                className="w-40 rounded-md border border-slate-200 bg-slate-50 py-1.5 pl-8 pr-3 text-sm outline-none transition focus:w-52 focus:border-teal-400 focus:bg-white focus:ring-2 focus:ring-teal-100"/>
            </div>
            {/* Context menu button */}
            <button onClick={()=>setShowMenu(s=>!s)}
              className={`grid h-9 w-9 place-items-center rounded-md transition ${showMenu?'bg-slate-100 text-slate-800':'text-slate-500 hover:bg-slate-100 hover:text-slate-700'}`}
              title="More options">
        ...
        {/* Call banner */}
        {callBanner && (
          <div className="flex items-center justify-between bg-teal-50 px-5 py-2.5 text-sm border-b border-teal-100">
            <span className="flex items-center gap-2 font-medium text-teal-800">
              {callBanner.kind==='video'?<Video size={16}/>:<Phone size={16}/>}
              {callBanner.kind==='video'?'Video':'Voice'} call started · connecting…
            </span>
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto px-2 py-4">
          ...
          <div className="mb-4 px-3">
            <div className={`grid h-12 w-12 place-items-center rounded-xl ${isDM&&dmUser?dmUser.color:'bg-slate-100 text-slate-500'} text-white`}>
              {isDM&&dmUser ? dmUser.initials : <Hash size={22}/>}
            </div>
          ...
          {sorted
            ...
            .map((m,i,arr)=>{
              const prev=arr[i-1];
              const grouped=prev&&prev.senderId===m.senderId&&parseFloat(m.t)-parseFloat(prev.t)<5*60000;
                ...
                <React.Fragment key={m.id}>
                  {showDay && fmtDay(m.t) && (
                    <div className="my-3 flex items-center gap-3 px-3">
                      ...
                      <span className="rounded-full border border-slate-200 px-3 py-0.5 text-xs font-semibold text-slate-500">{fmtDay(m.t)}</span>
                      <div className="h-px flex-1 bg-slate-100"/>
                    </div>
                  )}
        ...
        {/* Composer */}
        <Composer
          value={draft} setValue={handleTyping}
      ...
      {threadParent && (
        <ThreadPanel accounts={accounts} myId={myId} parent={threadParent}
          ...
          onSend={t=>sendMsg(t,threadParent.id)}
          onReact={(mid,e)=>toggleReact(mid,e,threadParent.id)}/>
      )}
    </div>
  );
}
  ...
  return (
    <div onMouseEnter={()=>setHover(true)} onMouseLeave={()=>{setHover(false);setPicker(false);}}
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="text-[15px] leading-relaxed text-slate-700 whitespace-pre-wrap">{renderText(msg.text)}</div>
        ...
        {!inThread && msg.threadCount>0 && (
          <button onClick={onThread} className="mt-1.5 flex items-center gap-2 rounded-md px-1.5 py-1 text-xs font-medium text-teal-600 hover:bg-white">
            <span className="flex -space-x-1">
              {[...new Set(msg.thread?.map(t=>t.senderId)||[])].slice(0,3).map(id=>{
                const tu=accounts[id]||{color:'bg-slate-400',initials:'?'};
                return <span key={id} className={`${tu.color} grid h-4 w-4 place-items-center rounded text-[8px] font-bold text-white ring-1 ring-white`}>{tu.initials}</span>;
              })}
            </span>
            {msg.threadCount} repl{msg.threadCount===1?'y':'ies'}
            <span className="text-slate-400">· last {fmtTime(msg.thread?.[msg.thread.length-1]?.t||msg.t)}</span>
          </button>
        )}
      </div>
      {hover && (
        <div className="absolute -top-3 right-3 flex items-center gap-0.5 rounded-lg border border-slate-200 bg-white p-0.5 shadow-sm">
          ...
          {canThread&&!inThread&&<button onClick={onThread} className="grid h-7 w-7 place-items-center rounded-md text-slate-500 hover:bg-slate-100" title="Reply in thread"><Reply size={15}/></button>}
      ...
      {picker && (
        <div className="absolute right-3 top-6 z-10 grid grid-cols-6 gap-0.5 rounded-lg border border-slate-200 bg-white p-1.5 shadow-lg">
    ...
    <div className="px-3 pb-3 pt-1">
      <div className="rounded-xl border border-slate-200 transition focus-within:border-teal-400 focus-within:ring-2 focus-within:ring-teal-100">
        ...
        <div className="flex items-center gap-0.5 border-b border-slate-100 px-2 py-1">
          <FmtBtn icon={<Bold size={14}/>} onClick={()=>wrap('**')}/>
          <FmtBtn icon={<Italic size={14}/>} onClick={()=>wrap('*')}/>
          <FmtBtn icon={<Code size={14}/>} onClick={()=>wrap('`')}/>
        </div>
        ...
        {attachments.length>0 && (
          <div className="flex flex-wrap gap-1.5 border-b border-slate-100 px-3 py-2">
            {attachments.map((f,i)=>(
              <div key={i} className="flex items-center gap-1.5 rounded-lg bg-slate-100 px-2.5 py-1 text-xs text-slate-700">
                <Paperclip size={12} className="text-slate-400"/>
                <span className="max-w-32 truncate font-medium">{f.name}</span>
                <span className="text-slate-400">({(f.size/1024).toFixed(0)}KB)</span>
                <button onClick={()=>onRemoveAttachment(i)} className="ml-1 text-slate-400 hover:text-rose-500"><X size={11}/></button>
              </div>
            ))}
          </div>
        )}
        ...
        <div className="relative flex items-center justify-between px-2 py-1.5">
          ...
          <button onClick={()=>onSend(value)} disabled={!canSend}
            className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-sm font-semibold transition ${canSend?'bg-teal-600 text-white hover:bg-teal-700':'bg-slate-100 text-slate-400'}`}>
            <Send size={14}/> Send
          </button>
          {showEmoji && (
            <div className="absolute bottom-11 right-2 z-10 grid grid-cols-6 gap-0.5 rounded-lg border border-slate-200 bg-white p-1.5 shadow-lg">
              {EMOJIS.map(e=><button key={e} onClick={()=>onEmoji(e)} className="grid h-8 w-8 place-items-center rounded-md text-lg hover:bg-slate-100">{e}</button>)}
            </div>
          )}
        </div>
      </div>
      <p className="mt-1 flex items-center gap-1 px-1 text-[11px] text-slate-400">
        <CornerDownLeft size={11}/> Enter to send · Shift+Enter for new line · **bold** *italic* `code`
      </p>
    </div>
  );
}
function FmtBtn({icon,onClick,title}){
  return <button onClick={onClick} title={title} className="grid h-7 w-7 place-items-center rounded-md text-slate-500 hover:bg-slate-100 hover:text-slate-700">{icon}</button>;
}
    ...
    <section className="flex w-96 shrink-0 flex-col border-l border-slate-200 bg-white">
      ...
      <div ref={ref} className="flex-1 overflow-y-auto py-2">
        ...
        <div className="my-2 flex items-center gap-3 px-4">
          <div className="h-px flex-1 bg-slate-100"/>
          <span className="text-xs font-medium text-slate-400">{parent.thread?.length||0} repl{(parent.thread?.length||0)===1?'y':'ies'}</span>
          <div className="h-px flex-1 bg-slate-100"/>
        </div>
        {(parent.thread||[]).map((tm,i)=>{
          const prev=(parent.thread||[])[i-1];
          const grouped=prev&&prev.senderId===tm.senderId&&parseFloat(tm.t)-parseFloat(prev.t)<5*60000;
          return <MsgRow key={tm.id} accounts={accounts} myId={myId} msg={tm} grouped={grouped} onReact={e=>onReact(tm.id,e)} canThread={false} inThread/>;
        })}
      </div>
      <div className="px-3 pb-3 pt-1">
